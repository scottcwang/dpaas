...
r = requests.post(
    ...
    json={
        ...
        'response_start_time': datetime.datetime.now(datetime.timezone.utc).timestamp(),
        'response_end_time': (datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=60)).timestamp()
    })
...
r = requests.post(
    ...
    json={
        ...
        'description': '# Title\nParagraph',
        'client_verify_key': client_verify_key_encoded,
        'response_start_time': (datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=60)).timestamp(),
        'response_end_time': (datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=120)).timestamp()
    })
assert r.status_code == 201
...
bad_voucher = ','.join([base64.urlsafe_b64encode(
    client_serial).decode(), secrets.token_urlsafe(17), str(datetime.datetime.now(datetime.timezone.utc).timestamp())])
bad_voucher_sign = client_signing_key.sign(bad_voucher.encode())
bad_voucher_encode = base64.urlsafe_b64encode(bad_voucher_sign).decode()
r = requests.get(url=hostname + '/' + str(collection_id) +
                 '/entry/' + bad_voucher_encode)
...
bad_voucher = ','.join([base64.urlsafe_b64encode(
    client_serial).decode(), entry_serial, str((datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(seconds=120)).timestamp())])
bad_voucher_sign = client_signing_key.sign(bad_voucher.encode())
bad_voucher_encode = base64.urlsafe_b64encode(bad_voucher_sign).decode()
r = requests.get(url=hostname + '/' + str(collection_id) +
                 '/entry/' + bad_voucher_encode)
