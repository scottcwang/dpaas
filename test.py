...
r = requests.post(url = hostname + '/' + str(future_collection_id) + '/token/entry')
assert r.status_code == 410 and r.json() == 'Not within collection interval'

r = requests.post(url = hostname + '/' + str(future_collection_id) + '/token/status')
assert r.status_code == 201
...
r = requests.get(url = hostname + '/' + str(future_collection_id) + '/entry/' + client_jwt)
assert r.status_code == 410 and r.json() == 'Not within collection interval'
...
r = requests.get(url = hostname + '/' + str(collection_id) + '/entry/' + bad_client_jwt)
...
r = requests.get(url = hostname + '/' + str(collection_id) + '/entry/' + client_jwt)
...
assert r.status_code == 400 and r.json() == 'Token contains nonce for non-entry action'
...
# cross-use a token
r = requests.post(hostname + '/' + str(collection_id) + '/enqueue/' + client_jwt)
assert r.status_code == 403 and r.json() == 'A corresponding session was not found'
